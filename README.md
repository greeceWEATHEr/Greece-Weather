...
const map = L.map("map").setView([40.6401, 22.9444], 10);
...
let marker = L.marker([40.6401, 22.9444])
  .addTo(map)
...
async function loadWeather() {
  ...
  try {
    ...
    for (
      let i = 0;
      i < data.hourly.time.length && shown < 24;
      i++
    ) {
      ...
      if (time >= now) {
        ...
        const [hourIcon] =
          describe(data.hourly.weather_code[i]);
